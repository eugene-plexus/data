...
`src/eugene_plexus_data/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_data" / "_generated"
WORKING_DIR = REPO_ROOT / ".codegen-cache"

SPECS_TARBALL_URL_TEMPLATE = "https://github.com/eugene-plexus/specs/archive/{ref}.tar.gz"

# data.yaml hosts the data component's path operations and a few local
# component schemas (DatasetManifest, TokenizerSpec, ImportRequest); it
# inlines its list/create request/response bodies under `paths` and
# $refs the shared schemas (ConfigDocument, Health, Problem, …) into
# components/common.yaml, which is a complete OpenAPI document hosting the
# shared schemas. We generate from both: common.yaml -> common_models.py
# for the shared schemas, and data.yaml -> models.py for the data-local
# schemas and path-inline shapes. The resulting modules include a handful
# of types the data component doesn't strictly need — that's fine, they're
# cheap and keep the generator's job simple.
SPECS_TO_GENERATE = [
    ("openapi/data.yaml", "models.py"),
...
def run_codegen(specs_root: Path, ref: str) -> None:
    ...
    for input_rel, output_name in SPECS_TO_GENERATE:
        ...
        cmd = [
            ...
            "--disable-timestamp",
            # data.yaml defines its list/create request/response shapes
            # inline under `paths`. Without `paths` scope the generator
            # would only emit the component schemas and miss the inline
            # response wrappers. Mirrors connector's / trainer's codegen,
            # which feed a paths-bearing spec the same way.
            "--openapi-scopes",
            "paths",
            "schemas",
        ]
