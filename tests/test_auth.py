"""Tests for v0.2 bearer auth on the data component.
...
from eugene_plexus_data.auth_state import AuthState
...
@pytest.fixture
def trainer_service_token(signing_key: bytes) -> str:
...
def test_missing_bearer_rejects_with_401(authed_client: TestClient) -> None:
    ...
    assert response.json()["detail"]["component"] == "data"
...
# --------------------------------------------------------------------------- #
# Operator audience accepted on operator routes + data routes
# --------------------------------------------------------------------------- #
...
def test_operator_token_accepted_on_datasets(
    authed_client: TestClient, operator_token: str
) -> None:
    response = authed_client.get(
        "/v1/data/datasets", headers={"Authorization": f"Bearer {operator_token}"}
    )
    assert response.status_code == 200


# --------------------------------------------------------------------------- #
# Service audience rejected on config, accepted on data reads
# --------------------------------------------------------------------------- #


def test_service_token_rejected_on_config_patch(
    authed_client: TestClient, trainer_service_token: str
) -> None:
    """A compromised peer must not be able to rewrite the data component's
    config (e.g. repoint the data root)."""
    response = authed_client.patch(
        "/v1/config",
        json={"defaultBlockSize": 256},
        headers={"Authorization": f"Bearer {trainer_service_token}"},
    )
    assert response.status_code == 401


def test_service_token_accepted_on_datasets(
    authed_client: TestClient, trainer_service_token: str
) -> None:
    """Normal production path: the trainer lists datasets/tokenizers using
    its service:trainer token."""
    response = authed_client.get(
        "/v1/data/datasets",
        headers={"Authorization": f"Bearer {trainer_service_token}"},
    )
...
def test_load_auth_state_disabled_when_no_signing_key() -> None:
    from eugene_plexus_data.auth_state import load_auth_state
