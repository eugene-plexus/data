"""Tests for the watchdog safe-mode contract on the data component.

Per specs/openapi/data.yaml: when started with
`EUGENE_PLEXUS_DAT_SAFE_MODE=1` the data component must
...
def safe_mode_settings(tmp_path: Path) -> Settings:
    ...
    config.write_text(
        yaml.safe_dump({"defaultBlockSize": 4096, "logLevel": "DEBUG"}),
        encoding="utf-8",
    )
...
def test_config_get_returns_defaults_not_disk_values(safe_mode_client: TestClient) -> None:
    ...
    assert response.status_code == 200
    body = response.json()
    # Disk had defaultBlockSize=4096 and logLevel=DEBUG; safe mode must
    # serve the built-in defaults instead.
    assert body.get("logLevel") == "INFO"
    assert body.get("defaultBlockSize") != 4096
