...
class DatasetManifest(BaseModel):
    """
    v0.3-draft. Describes a prepared dataset: its source, pretokenization
    parameters, and the vocab fingerprint that must match the tokenizer +
    any consumer. Referenced from `common.yaml#/components/schemas/DatasetRef`.
    ...
    vocabFingerprint: str | None = Field(
        None,
        description='SHA256 of the tokenizer used to pretokenize; must match consumers.',
    )
...
class TokenizerSpec(BaseModel):
    """
    v0.3-draft. A trained BPE tokenizer + its vocab fingerprint.
    """

    tokenizerId: UUID
    name: str
    vocabSize: int | None = None
    vocabFingerprint: str | None = None
    status: Status1 | None = None


class ImportRequest(BaseModel):
    """
    v0.3-draft. Where to import raw data from.
    """

    sourceKind: SourceKind
    location: str = Field(
        ...,
        description='Filesystem path, URL, or HuggingFace dataset id depending on sourceKind.',
    )
    dataColumn: str | None = Field(
        None, description='For structured sources, the text column to extract.'
    )


class V1DataDatasetsGetResponse(BaseModel):
...
class V1DataDatasetsDatasetIdPretokenizePostRequest(BaseModel):
    tokenizerId: UUID
    blockSize: int = Field(..., ge=1)


class V1DataTokenizersGetResponse(BaseModel):
    tokenizers: list[TokenizerSpec]


class V1DataTokenizersTrainPostRequest(BaseModel):
    name: str
    vocabSize: int = Field(..., ge=1)
    minFrequency: int | None = Field(None, ge=1)
    sourceDatasetIds: list[UUID] | None = None
