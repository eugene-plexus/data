...
class Settings(BaseSettings):
    ...
    built-in defaults. Set by the watchdog via EUGENE_PLEXUS_DAT_SAFE_MODE=1
    ...
    specs/openapi/data.yaml: while safe mode is active the data component
    refuses mutating operations and reports degraded health, but config
    ...
    spawn time (EUGENE_PLEXUS_DAT_AUTH_SIGNING_KEY). When absent the
    component runs unauthenticated — dev / standalone path only."""

    service_token: str | None = None
    """Long-lived service JWT (EUGENE_PLEXUS_DAT_SERVICE_TOKEN). Captured
    here for symmetry with other component kinds and any future outbound
    peer calls (e.g. notifying the trainer that a dataset is ready)."""

    master_key: str | None = None
    """Base64-encoded 32-byte secretbox key (EUGENE_PLEXUS_DAT_MASTER_KEY).
    Not used in the v0.3 skeleton — no data-component secrets are encrypted
    at rest yet. Reserved."""
