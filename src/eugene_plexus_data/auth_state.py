"""Auth state for the data component's verify-only role.

Built once at startup from the env vars the watchdog threads in when
it spawns this child:

  * `EUGENE_PLEXUS_DAT_AUTH_SIGNING_KEY` — base64 of the 32-byte HMAC
    key used to validate inbound bearer tokens.
  * `EUGENE_PLEXUS_DAT_SERVICE_TOKEN` — long-lived JWT (`aud:
    service:data`). The watchdog supplies this for symmetry with other
    kinds; a future engine can present it for outbound peer calls (e.g.
    notifying the trainer that a dataset is ready).
  * `EUGENE_PLEXUS_DAT_MASTER_KEY` — base64 of the 32-byte secretbox
    key. Not used in the v0.3 skeleton — no data-component secrets are
...
def load_auth_state(
    *,
    ...
    if signing_key is None:
        ...
        log.warning(
            "EUGENE_PLEXUS_DAT_AUTH_SIGNING_KEY not set — running unauthenticated "
