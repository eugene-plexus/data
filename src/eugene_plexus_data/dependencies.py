...
For the data component: dataset/tokenizer operations (`/v1/data/*`)
accept operator OR service tokens — the trainer/coordinator presents a
`service:*` token to list datasets and tokenizers, and operators drive
imports/training from the UI. Config edits are operator-only.
"""
...
def require_authorized(
    ...
    """Operator OR any service-audience token accepted — for /v1/data/*
    dataset and tokenizer operations, which peer services (trainer,
    coordinator) drive and the operator may drive through the UI."""
