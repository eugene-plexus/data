...
async def healthz(request: Request) -> Health:
    ...
    # reachable. When the preparation engine isn't initialized (safe mode,
    # bad config, or — in the v0.3 skeleton — because the engine isn't
    # built yet), surface that as `degraded` so consumers can tell the
    # component is alive but can't prepare data until it's fixed.
