...
async def restart() -> RestartResult:
    ...
    Mirrors the orchestrator/hemisphere-driver restart endpoints. The data
    component only re-reads `requiresRestart: true` config keys (logLevel,
    port, …) at startup; this is the UI's mechanism for completing a
    config-change flow.
    """
